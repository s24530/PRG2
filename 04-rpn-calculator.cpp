...
auto evaluate_reszta(std::stack<double>& stack) -> void
{
    if(stack.size()<2)
    {
        throw std::logic_error{"not enough operands for %"};
...
auto evaluate_pow(std::stack<double>& stack) -> void
{
    if(stack.size()<2)
    {
        throw std::logic_error{"not enough operands for pow"};
    }
    auto const b = pop_top(stack);
    auto const a = pop_top(stack);
    stack.push(pow(a,b));
...
auto evaluate_calkowite(std::stack<double>& stack) -> void
{
    if(stack.size()<2)
    {
        throw std::logic_error{"not enough operands for //"};
